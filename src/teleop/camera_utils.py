...
class AsyncCameraReader:
    """LeRobot-style async camera reader with background thread and timeout handling - ROS2 version"""

    def __init__(self, node, image_height, image_width, timeout_ms=200, slow_speed_threshold=15.0):
        self.node = node  # ROS2 node instance
        ...
        self.latest_frames = {}
        self.timestamps = {}  # ROS header timestamps
    ...
    def _setup_subscribers(self):
        """Setup ROS2 subscribers for 3 cameras only: top, left wrist, right wrist"""
        # Camera topic mapping - only 3 RGB cameras
        # camera_0: ZED 2i top camera (publishes to /camera_0/zed_node/rgb/image_rect_color)
        # camera_3: RealSense left wrist (publishes to /camera_3/color/image_raw)
        # camera_4: RealSense right wrist (publishes to /camera_4/color/image_raw)
        camera_topics = {
            'top_camera': '/camera_0/zed_node/rgb/image_rect_color',  # ZED 2i top camera
            'left_wrist': '/camera_3/color/image_raw',                # RealSense left wrist
            'right_wrist': '/camera_4/color/image_raw',               # RealSense right wrist
        }
        ...
        for cam_name, topic in camera_topics.items():
            sub = self.node.create_subscription(
                Image,
                topic,
                lambda msg, name=cam_name: self._optimized_image_callback(msg, name),
                10
            )
            self.subscriptions.append(sub)
    
    def _optimized_image_callback(self, data, cam_name):
        """Optimized callback for RGB cameras only - ROS2 version"""
        try:
            # Convert image using CvBridge - RealSense publishes in RGB8 format
            ...
            with self.frame_lock:
                ...
                # Calculate full timestamp as float (ROS2 uses sec and nanosec)
                frame_timestamp = data.header.stamp.sec + data.header.stamp.nanosec * 1e-9
...
class OptimizedImageRecorder:
    ...
    def _setup_subscribers(self):
        ...
        camera_topics = {
            'cam_base': '/camera/color/image_raw',
